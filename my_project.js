"use strict"
if (b4w.module_check("my_project_main"))
...
/////////////////////////////////////////////
    ...
    <div class="onoffswitch" onclick= fullscreen()>
       ...
       //console.log("init_controls Begin OK");
        ...
        init_buttons();
        //document.getElementById("load-1"), (function
        /*document.getElementById("exit").addEventListener("click", function (e) {
            console.log("Exit = ");
            if (!EndGame && GameBegin && !not_select)
    ...
    function _showImage() {
        ...
        const elem2 = document.getElementById("load-2");
            // Переключаем класс show-before
        elem2.classList.toggle("show-before");
        const elem3 = document.getElementById("load-3");

        elem3.classList.toggle("show-before");
        const elem4 = document.getElementById("load-4");

        elem4.classList.toggle("show-before");        
        const elem5 = document.getElementById("load-5");

        elem5.classList.toggle("show-before");
        const elem6 = document.getElementById("load-6");

        elem6.classList.toggle("show-before");
    }
    function _enable_fullsrc(m_screen, db) {
        const elem = document.getElementById("load-7");
...
//////////////////////////////////////////////    
    ...
    function load_cb(data_id, success) {
       //console.log("function load_cb");
        ...
        m_Pins_arr2 = [...m_Pins_arr2, pin2Trans];
        ...
        if (move == true) {
            if (_moving_obj) {
               ...
               //console.log("_moving_obj");
                ...
                const rnd_transInt2 = ((Math.random() - .5) * 2) * 5;

                const razn = (_last_posX_R - _last_posX_L) / 1000;
...
//##### 11 #####
        if (FrameTrue == 11) {
           //console.log("if (FrameTrue == 11) {");
            ...
            const elem = document.getElementById("load-1");
...
///////////////////////////////////
    ...
    function calculate_velocities(pin_name, curr_pos, curr_quat) {
        ...
        function getAxisAngle(quat) {
            const angle = 2 * Math.acos(quat[3]); // Кватернион обычно представлен как [x, y, z, w]
            const s = Math.sqrt(1 - quat[3] * quat[3]); // s = sin(angle/2)
            const axis = s < 0.001
...
function main_canvas_move(e) {
  ...
  //если броска еще нет(move=false) и коорд.мыши ограничены по x(в обе стороны) и по y в одну
                        ...
                        //if (move == false && y < 220.0 && y > 0.0) { select_trans[1]>-11.0 && 
                            ...
                            //console.log("_last_posX_R=" + _last_posX_R + "  _last_posX_L=" + _last_posX_L);
                        }
